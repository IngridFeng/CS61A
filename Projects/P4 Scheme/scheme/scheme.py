...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        if type(p) is MacroProcedure:
            return scheme_eval(p.apply_macro(rest, env), env)
        check_procedure(p)
        return scheme_apply(p, rest.map(lambda op: scheme_eval(op, env)), env)
        # END PROBLEM 5
...
def eval_all(expressions, env):
    ...
    return scheme_eval(buffer.first, env, True)
    # END PROBLEM 8
...
def do_and_form(expressions, env):
    ...
    # BEGIN PROBLEM 13
    curr, buffer = True, expressions
    while buffer is not nil:
        if buffer.second is nil:
            curr = scheme_eval(buffer.first, env, True)
        else:
            curr = scheme_eval(buffer.first, env)
...
def make_let_frame(bindings, env):
    ...
    buffer = bindings
    while buffer is not nil:
        check_form(buffer.first, 2, 2)
        fs = Pair(buffer.first.first, fs)
        vs = Pair(scheme_eval(buffer.first.second.first, env), vs)
        buffer = buffer.second
    check_formals(fs)
    return env.make_child_frame(fs, vs)
    # END PROBLEM 15
...
def do_define_macro(expressions, env):
    ...
    if not isinstance(name, Pair) or not scheme_symbolp(name.first):
        raise SchemeError("wrong expressions")
    env.bindings[name.first] = MacroProcedure(name.second, expressions.second, env)
    return name.first
    # END Problem 21
