...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        if player == 0: #the turn for player0
            score0 += take_turn(strategy0(score0, score1), score1, dice) #score update
            if is_swap(score0, score1): #swap if swine swap is satisfied
                score0, score1 = score1, score0
        else: #the turn for player1
            score1 += take_turn(strategy1(score1, score0), score0, dice)
            if is_swap(score1, score0): #swap if swine swap is satisfied
                score0, score1 = score1, score0
        player = other(player)
    # END PROBLEM 5
    # BEGIN PROBLEM 6
        if turn == 0:
            curr = say(score0,score1)
        else:
            curr = prev(score0, score1)
        prev = curr #keep track of the return of this turn
        turn += 1
    # END PROBLEM 6
...
def announce_highest(who, previous_high=0, previous_score=0):
    ...
    def say_high(score0, score1):
        ...
        if curr_incr > previous_high:
            print (curr_incr, "point(s)! That's the biggest gain yet for Player", who)
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    """
    # BEGIN PROBLEM 9
    max_score_roll, max_score = 0, 0
    for i in range(1, 11):
        curr = make_averaged(roll_dice, num_samples)(i, dice)
        if curr > max_score:
            max_score_roll, max_score = i, curr
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    should_swap = is_swap(new_score,opponent_score) and (new_score < opponent_score)
    if should_swap or free_bacon(opponent_score) >= margin:
        return 0
    return num_rolls
    # END PROBLEM 11
