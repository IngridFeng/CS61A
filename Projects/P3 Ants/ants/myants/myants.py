...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        # BEGIN Problem 7
        # ant = self.place.ant
        # if (ant is not None) or (ant is not None and ant.blacks_path == True):
        #     return False#(ant is not None) and (ant is None or ant.blacks_path == True)
...
class LongThrower(ThrowerAnt):
    ...
    def nearest_bee(self, hive):
        ...
        while not track.bees and counter >= 0:
            counter -= 1
            track = track.entrance
        if track == hive:
            return
